...
fn report() -> Result<serde_json::Value> {
    Ok(serde_json::json!({
        "active": mlxc::memory::active()?,
        "cache": mlxc::memory::cache()?,
        "peak": mlxc::memory::peak()?,
        "limit": mlxc::memory::limit()?,
    }))
}
...
fn clear_cache() -> Result<(usize, usize)> {
    let before = mlxc::memory::cache()?;
    mlxc::memory::clear_cache().context("clearing the cache failed")?;
    Ok((before, mlxc::memory::cache()?))
}
...
fn set_limit(bytes: usize) -> Result<usize> {
    Ok(mlxc::memory::set_limit(bytes).context("setting the memory limit failed")?)
}

/// Forgets the high-water mark, so a later reading is about what follows.
fn reset_peak() -> Result<()> {
    Ok(mlxc::memory::reset_peak().context("resetting the peak failed")?)
}
