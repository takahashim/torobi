...
module Torobi
  ...
  #
  # One emission is one `Shape::Inference`: what it reads, and where to
  # say a refusal happened. The rules are its methods, so each reads
  # those rather than being handed them. What is left here at the top is
  # dimension arithmetic the DSL also asks for, which needs no emission.
  module Shape
    # The rules `Inference` implements, which must be exactly the set
    # `config/ops.yml` names. Declared rather than left implicit in the
    # methods, so the two can be compared instead of remembered.
    RULES = %i[
      parameter same_as_input broadcast transpose reshape slice reduce
      matmul take sdpa cross_entropy cast
    ].freeze

    # [shape, dtype] for one emission.
    def self.infer(rule, inputs:, params:, attrs:, where:)
      Inference.new(inputs:, params:, attrs:, where:).call(rule)
    end
    ...
    def self.divide!(dim, count, into:, where:)
      given = count
    ...
    # `axis` counted from the front, however it was written.
    def self.axis!(axis, rank, where:)
    ...
    # One emission's shape and dtype: the handles it reads, the parameters
    # it uses, its attributes, and `where` for what it says on refusing.
    class Inference
      def initialize(inputs:, params:, attrs:, where:)
        @inputs = inputs
        @params = params
        @attrs = attrs
        @where = where
      ...
      # [shape, dtype] by `rule`, one of RULES.
      def call(rule)
      ...
      # Where this emission is, for what a refusal says.
      attr_reader :where

      # The input most rules are about.
      def first = @inputs.first

      def unify_dim(a, b, axis:, where: @where)
        return b if a.nil?
      ...
      def axis!(axis, rank) = Shape.axis!(axis, rank, where: @where)

      # --- rules ---

      def parameter = [@params.fetch(0).shape, @params.fetch(0).dtype]

      def cast = [first.shape, IR::Dtype.check!(@attrs.fetch("dtype").to_sym, where: @where)]

      def same_as_input
        [first.shape, same_dtype]
      end
      ...
      def reshape
        ...
        shape, dtype = rest_of_reshape(from.drop(keep), target.drop(keep), kept: keep)
      ...
      # a formality.
      def rest_of_reshape(from, target, kept: 0)
        ...
        if from.include?(nil)
          ...
          return [target.map { |d| d == -1 ? nil : d }, first.dtype]
        end
        ...
        [target.map { |d| d == -1 ? total / known : d }, first.dtype]
      end

      def broadcast
        a, b = @inputs
        [broadcast_shapes(a.shape, b.shape), same_dtype]
      end
      ...
      def transpose
        ...
        [axes.map { |i| shape[i] }, first.dtype]
      end

      def slice
        shape = first.shape.dup
        axis = axis!(@attrs.fetch("axis"), shape.size)
      ...
      def reduce
        ...
        axes = @attrs.fetch("axes")&.map { |a| axis!(a, shape.size) } || (0...shape.size).to_a
      ...
      def sdpa
        ...
        batch = unify_dim(q[0], unify_dim(k[0], v[0], axis: 0), axis: 0)
      ...
      def cross_entropy
        ...
        [unify(logits.shape[0..-2], targets.shape), logits.dtype]
      end
    end
  end
end
