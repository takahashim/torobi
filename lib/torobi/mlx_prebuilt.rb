...
# pre-built MLX instead, with mlx-c already built into it: the engine binds
# mlx-c's headers and links its archive straight from the prefix
# (engine/build.rs, and docs/vendoring.md "mlx-c, bound here"). This
# fetches that MLX once, checks the bytes against a recorded digest, and
# says where the prefix landed, which is all a build is told.
#
...
module MlxPrebuilt
  ...
  # The generation the engine's binding was written against, keyed the way
  # the manifest spells it. A different question from what the archive
  # holds: the archive is what was built, this is which mlx-c the calls in
  # engine/src/mlxc/ were chosen for and checked against. bindgen reads
  # the archive's own headers, so a different mlx-c would still link; what
  # it could do is compile and mean something else. Empty for a pin that
  # predates the field.
  #
  # `mlx-c` is a commit, not the `v0.6.0` tag: mlx-c's tag pins MLX 0.31.1
  # and seven later commits pin 0.32.2, and they change headers under
  # `mlx/c/`. A manifest naming the tag is the wrong build even though it
  # says 0.6.0.
  ...
  # Whether the archive is the generation the binding was written for.
  #
  ...
  def check_version(said)
    ...
    unless said
      raise Refused, "#{asset} has no MANIFEST.txt, so which MLX it holds cannot " \
                     "be checked against the binding (#{required.inspect})"
    end
    ...
    required.each do |name, want|
      ...
      raise Refused, "#{asset} holds #{name} #{got.empty? ? "(unstated)" : got}, " \
                     "but the binding wants #{want}. Build takahashim/mlx-prebuilt " \
