...
per element, not allocation (a version writing into a preallocated
`IO::Buffer` measured 32 MB/s, which is not worth an experimental API).
Everything else here (Thrift, the levels, the values) is a rounding
error beside it.
