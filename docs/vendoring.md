...
| MLX core | **not built from source here**: there is no Metal compiler on this machine, so a pre-built archive is used instead. `takahashim/mlx-prebuilt`, built from stated inputs on a runner with the toolchain rather than taken from a third party's release. It must carry the generation the binding was written against - **MLX 0.32.2 over mlx-c `c74db5307cc8`** - and `mlx_prebuilt.json`'s `requires` is what holds it to that. It names the mlx-c **commit**, not `v0.6.0`: the tag pins MLX 0.31.1 and seven later commits pin 0.32.2, and those commits change headers under `mlx/c/`, so a build from the tag is the wrong generation even though it says 0.6.0. The pin names **`takahashim/mlx-prebuilt` `v0.6.0.2`** (MLX 0.32.2 / mlx-c `c74db530`), which is what a build fetches: `macos-arm64` (digest `290331d6…`) and `linux-x86_64`, the `linux-x86_64-cuda12` archive (CUDA 12.9, sm 80/86/89/90a, digest `060f2757…`). The release also carries the Metal kernels as an asset of their own, `<stem>.metallib.tar.gz` (digest `20d3b7e1…`), which a platform gem fetches on first use. `rake mlx:pin` checks the bytes of the archive it records, and CI's `linux` job repeats the fetch, verify, bind and link for the Linux one |
| mlx.metallib | 129 MB. MLX locates it through `dladdr`, i.e. **beside whichever library holds the MLX symbols**: the installed bundle, or `lib/torobi/` for a checkout. It comes from the prefix at `<prefix>/lib/mlx.metallib`; `ext/torobi/extconf.rb` installs it beside the bundle, `rake metallib` copies it into the checkout, and `engine/build.rs` links it beside the engine's own binaries. A source install keeps that: it is beside the bundle and MLX finds it. The macOS platform gem cannot ship it and may not be able to write beside the installed bundle, so on first use it is fetched into `~/.cache/torobi/` and MLX is told where it is (`mlx_metal_set_metallib_path`, mlx-swift issue #415), rather than into the gem's own directory |
| Linux | the same binding, the same bindgen blocklist, and a link against the CUDA toolkit (cudart, cuBLASLt, cuFFT, NVRTC, the driver), cuDNN and **OpenBLAS**, which MLX's CPU backend calls for BLAS and LAPACK and which its own CMake package names. Checked in an amd64 container with CUDA 12.9 and cuDNN 9 against the archive mlx-prebuilt's CI built: 650 functions bound, none of them half-precision, and the engine's test binary and command line link and start. A machine that runs it needs `libopenblas0` as well as the driver. **At run time MLX compiles the CUDA kernels it did not build ahead of time through NVRTC, which reads NVIDIA's headers (`include/cccl`, `include/cute`, `include/cutlass`) from one directory above the extension; the Linux platform gem ships them in `lib/include`, and a checkout gets them from the prefix; the engine refuses before a step when they are not there.** The `linux` job in CI fetches and checks the pinned archive and repeats the bind and link on a GitHub runner, with no GPU; it first passed on 2026-09-23 |
