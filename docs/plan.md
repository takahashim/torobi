...
支配的なのは snappy だけで (Plain の値化は 0.6ms)、しかも**要素ごとの Ruby の解釈コスト
が底**である。ホットループを 1 つに畳んで +12%。

`IO::Buffer` は**測り方に騙された**。片方ずつ測ると +18% (32 MB/s) に見えたので採用
しかけたが、**1 プロセス内で交互に測ると 10% 遅い** (0.117s 対 0.129s)。先に測った側が
不利になっていただけで、比較になっていなかった。確保が底ではないので、確保を無くしても
速くならない。experimental かどうか以前に、**利得が無い**。
